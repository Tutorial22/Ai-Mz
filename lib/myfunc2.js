...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./session/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./session/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./session/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./session/${filename}.mp4`)
	Promise.all([unlink(`./session/${filename}.mp4`), unlink(`./session/${filename}.gif`)])
	return buffer5
				   }